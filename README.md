...
This plugin has been created as a consequence of being part of the Green Web Foundation GAW project [Introducing our grid-aware websites project](https://www.thegreenwebfoundation.org/news/introducing-our-grid-aware-websites-project/). You can visit the article on Branch Magazine to better understand the context, how the plugin works and what it tries to achieve.

**DISCLAIMER**: This is an experimental plugin in alpha stage, so be aware of its limitations and do not use it on production sites.
...
- **Medium Intensity**: A smaller, slightly blurred version is displayed with alt text, explanatory message, and a button to load the original image
- **High Intensity**: Images don't load by default. Instead, a placeholder with informative text and a button to load the content is displayed, along with alt text
...
- **High Intensity**: Videos don't load by default. Instead, a placeholder with informative text and a button to load the content is displayed, along with the video title
...
* Overall feelings about usability of the plugin.
* Wording changes for more clearness are more than welcome.
...
* Decide if remove lite-youtube (a lightweight Youtube player) assets (used on early stages but not now) or think if could be beneficial on some cases.
* Decide when to load lite-youtube (now it's implemented) it does not look to add anything extra since we already use placeholders and it looks like it needs to load the whole player JS anyway.
* Decide if apply the lazy-load with [IntersectionObserver API](https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API)
